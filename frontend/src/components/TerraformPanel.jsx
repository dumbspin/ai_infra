...
import { Box, ChevronDown, ChevronRight, Copy, Check } from 'lucide-react';

export default function TerraformPanel({ terraformCode }) {
    ...
    <div className="bg-dark-800 rounded-xl border border-gray-800 shadow-xl overflow-hidden">
      ...
      >
        <div className="flex items-center space-x-2 text-sm font-semibold text-white">
          ...
          <span className="text-xs text-gray-400 font-mono font-normal">(main.tf)</span>
        </div>
        <div className="flex items-center space-x-2 text-gray-400">
          <span className="text-xs font-mono">{isOpen ? 'Hide HCL' : 'Expand HCL'}</span>
      ...
      {isOpen && (
        <div className="p-4 bg-dark-900/80 relative">
          ...
          <pre className="text-xs font-mono text-purple-300 overflow-x-auto p-4 bg-black/40 rounded-lg border border-gray-800 max-h-80 leading-5">
            {terraformCode || '# No Terraform HCL generated yet'}
          </pre>
        </div>
      )}
    </div>
